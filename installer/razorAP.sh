...
if [ $? = 0 ]; then
	echo '\e[33mTodo generado correctamente'
	echo '\e[33mLevantando hosatpd-wpe'
	echo "\e[33mSe va a ejecutar: hostapd-wpe $pwd/$cert/$cert.conf -s"
	echo
	...
	hostapd-wpe $pwd/$cert/$cert.conf
	if [ $? = 0 ]; then
		echo '\e[36mSistema levantado correctamente\e[39m'
		echo
	else
		echo '\e[31mError ejecutando: hostapd-wpe $pwd/$cert/$cert.conf -s , intentarlo manualmente.\e[39m'
		echo
	fi
else
